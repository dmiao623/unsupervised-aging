...
@app.cell
def _(Path, os):
    features_df_path = Path(os.environ["UNSUPERVISED_AGING"]) / "data/feature_matrices/2025-07-22_kpms-v3-supervised_feature-matrix.csv"
    xcats_path       = Path(os.environ["UNSUPERVISED_AGING"]) / "data/feature_matrices/2025-07-22_kpms-v3-supervised_xcats.json"
    results_df_path  = Path(os.environ["UNSUPERVISED_AGING"]) / "data/model_evaluation_results/2025-07-22_kpms-v3-supervised_results_[xcat]"
...
def _(features_df_path, json, pd, results_df_path, xcats_path):
    ...
    results_df = pd.concat((
        pd.read_csv(str(results_df_path).replace("[xcat]", xcat), low_memory=False) for xcat in xcats.keys()
    ), axis=0)
    xcats.keys()
...
def _(plot_grouped_model_performance_boxplot, results_df):
    def plot_ycat_kpmsv1_supunsup_results(ycat: str, ylabel: str, **kwargs):
        g = plot_grouped_model_performance_boxplot(
            ...
            {
                "kpms-v2_nonmeta": "unsup. w/ metasyllables",
...
def _(Optional, Tuple, linregress, np, pd, plt, results_df):
    ...
    plot_pred_vs_true(results_df[results_df["model"] == "XGBoost"], "kpms-v2_nonmeta", "age", lims=(0, 200))
    plot_pred_vs_true(results_df[results_df["model"] == "XGBoost"], "kpms-v2_nonmeta", "fi", lims=(0, 14))
    return
